...
class TaskPacketTests(unittest.TestCase):
    ...
    def test_write_packet_creates_issue_directory(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertIn('"issue": 44', path.read_text(encoding="utf-8"))

    def test_out_of_scope_and_dependencies_are_recognized(self):
        issue = self.issue(
            "## Out of scope\n- unrelated refactor\n\n"
            "## 依存\n- #43\n"
        )
        packet = task_packet.build_packet(issue, [])
        self.assertEqual(packet.out_of_scope, ["unrelated refactor"])
        self.assertEqual(packet.dependencies, ["#43"])
